...
function startGame(playerName, playerSrcImage){
    ...
    setTimeout(() => {
        letter.style.color = 'rgba(26,59,90,0.9)';
        question.style.color = '#2D2D2D';
        letter.classList.add('jello-horizontal');

        // Empiezan las preguntas

        const newRandomSet = createRandomSetFrom(questionsDeck);
        renderNextQuestion(newRandomSet, letterElement, questionElement);

        inputButton.addEventListener('click', ()=> {
            const currentCard = newRandomSet[cardIndex];
...
function renderNextQuestion(questionsSet, letterElement, questionElement){
    const currentCard = questionsSet[cardIndex]; //este cardIndex tiene valor inicial de 0
